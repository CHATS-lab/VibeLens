"""Tests for context batcher module.

Tests chain merging, affinity-based packing, budget constraints,
oversized sessions, and explicit max_batch_tokens parameter.
"""
...
def test_single_session_single_batch() -> None:
    ...
    batches = build_batches([ctx], max_batch_tokens=100_000)
...
def test_multiple_sessions_single_batch() -> None:
    ...
    batches = build_batches(contexts, max_batch_tokens=100_000)
...
def test_empty_input() -> None:
    ...
    assert build_batches([], max_batch_tokens=100_000) == []
...
def test_explicit_batch_tokens_parameter() -> None:
    """When max_batch_tokens is passed, it controls batch splitting."""
    contexts = [_make_context(f"s{i}", text_length=10_000) for i in range(3)]
    batches = build_batches(contexts, max_batch_tokens=2_000)

    assert len(batches) == 3
    total = sum(len(b.contexts) for b in batches)
    assert total == 3
    print(f"Explicit-parameter batching: {len(batches)} batches from 3 sessions")
