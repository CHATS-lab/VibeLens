...
export function PromptNavPanel({
  ...
  if (collapsed) {
    return (
      <div
        ...
        className={`hidden xl:flex relative shrink-0 h-full flex-col items-center border-l border-accent-cyan ${NAV_BG} py-3`}
      >
        <button
          onClick={() => onCollapsedChange(false)}
          className="p-1.5 text-accent-cyan hover:text-accent-cyan hover:bg-accent-cyan-muted rounded transition"
  ...
  if (hasFlowPhases || hasFlowSections) {
    ...
    return (
      <div
        style={{ width }}
        className={`hidden xl:flex relative shrink-0 h-full flex-col border-l border-accent-cyan ${NAV_BG}`}
      >
            ...
            {sections.map((section, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-[11px] text-muted pl-4">
    ...
    >
      ...
      {hasPrompts && hasSubAgents && (
        <div className="shrink-0 px-3 pb-2">
          <div className="flex gap-1 bg-accent-cyan-muted rounded-md p-1">
            <Tooltip text="Navigate user prompts and plan entries" className="flex-1 min-w-0">
              <button
                ...
                className={`w-full flex items-center justify-center text-xs py-1.5 rounded transition ${
                  navMode === "prompts"
                    ? "bg-accent-cyan-subtle text-accent-cyan font-semibold"
...
function NavHeader({ onCollapse }: { onCollapse: () => void }) {
  return (
    <div className="shrink-0 flex items-center justify-between px-3 pt-3 pb-2 border-b border-accent-cyan">
      <div className="flex items-center gap-1.5">
        <Compass className="w-3.5 h-3.5 text-accent-cyan" />
        <span className="text-[11px] uppercase tracking-wider text-accent-cyan font-semibold">
          Navigation
        </span>
      </div>
      <button
        onClick={onCollapse}
        className="p-1 text-accent-cyan hover:text-accent-cyan hover:bg-accent-cyan-muted rounded transition"
